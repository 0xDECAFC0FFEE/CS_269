...
from    src.models.smaml_learner import Learner
...
class SparseMeta(nn.Module):
    ...
    def forward(self, x_spt, y_spt, x_qry, y_qry):
        ...
        for i in range(task_num):
            ...
            with torch.no_grad():
                # [setsz, nway]
                logits_q = self.net(x_qry[i], {k:v for k,v in self.net.named_parameters()}, bn_training=True)
            ...
            for k in range(1, self.update_step):
                ...
                for name, param in fast_weights.items():
                    if "weight" in name:
                        l1_learner += torch.norm(param, 1)
                loss += self.lamda * l1_learner
        ...
        for name, param in self.net.named_parameters():
            if "weight" in name:
                l1_meta += torch.norm(param, 1)
        loss_q = losses_q[-1] / task_num + self.lamda * l1_meta
    ...
    def finetunning(self, x_spt, y_spt, x_qry, y_qry):
        ...
        with torch.no_grad():
            # [setsz, nway]
            logits_q = net(x_qry, {k:v for k,v in self.net.named_parameters()}, bn_training=True)
            # [setsz]
            pred_q = F.softmax(logits_q, dim=1).argmax(dim=1)
            # scalar
